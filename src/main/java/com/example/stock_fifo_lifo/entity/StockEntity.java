...
public class StockEntity {
    ...
    public Stock stockAvailable(Connection connection, Timestamp initialDate, Timestamp finalDate, String id_item, String id_store) throws Exception {
        ...
        try{
            ...
            double PUMP = 0.0;
            double total = 0.0;
            PUMP = stock_final.getTotal() / stock_final.getInitial_quantity();
            total = stock_final.getTotal();

            stock.setInitial_quantity(stock_initial.getInitial_quantity());
            stock.setRest_quantity(stock_final.getInitial_quantity());
            stock.setAverage_unit_price(PUMP);
    ...
    private String getNameItem(Connection connection, String id_item) throws Exception {
        int id_name_length = id_item.length();
        System.out.println(id_name_length);
        String name = null;
        String item = id_item+"%";
        Boolean close_connection = false;
        if(connection == null) {
    ...
    public Stock stockAvailableOnDate(Connection connection, Timestamp date, String id_item, String id_store) throws Exception {
        Boolean close_connection = false;
        //System.out.println(date + " date" + id_item + " id_item" + id_store + " id_store");
        double total = 0.0;
        double quantity = 0.0;
        double average = 0.0;
        if(connection == null) {
        ...
        try{
            ...
            //System.out.println(stock.getInitial_quantity() + " initial quantity");
            Stock stock1 = stockEntity.totalOut(null, id_item, id_store, date);
            //System.out.println(stock1.getInitial_quantity() + " initial quantity");
    ...
    public static void main(String[] args) throws Exception{
        ...
        stockEntity.getNameItem(null, "IR00002");
    }
}
